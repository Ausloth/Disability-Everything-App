...
const MOCK_ACTIVITIES = [
  { id: 'act-1', name: 'Community Gardening', category: 'Outdoors', location: 'Box Hill', level: 'Medium Support', emoji: '🌱' },
  { id: 'act-2', name: 'Art Therapy', category: 'Creative', location: 'Onsite', level: 'Low Support', emoji: '🎨' },
  { id: 'act-3', name: 'Bowling', category: 'Physical', location: 'Strike Eastland', level: 'Medium Support', emoji: '🎳' },
  { id: 'act-4', name: 'Cooking Masterclass', category: 'Life Skills', location: 'Onsite Kitchen', level: 'Medium Support', emoji: '🍳' },
]
...
const MOCK_OUTLINES: ActivityOutline[] = [
  {
    ...
    weeks: Array.from({ length: 24 }).map((_, i) => ({
      id: `w-${i+1}`,
      weekNumber: i + 1,
      variationNote: i === 7 ? 'Barefoot Lawn Bowls' : i === 14 ? 'Night Glow Bowling' : '',
      goalProgression: 'Social participation, motor skills, turn-taking.',
      tasksContent: 'Travel to venue. Rent shoes. 1-2 games of bowling. Lunch at venue cafe.',
      locationTarget: i === 7 ? 'Local Bowls Club' : 'Strike Eastland',
      materialsCost: '$15 for bowling + $10 for lunch',
      staffGuide: 'Ensure clients are wearing appropriate socks. Assist with score entry and choosing the right weight ball.'
    }))
  },
  {
    id: 'out-2',
    ...
    weeks: Array.from({ length: 24 }).map((_, i) => ({
      ...
      tasksContent: i === 0 ? 'Planning the beds and buying seeds.' : i === 12 ? 'Weeding and pest management.' : 'Harvesting and cooking preparation.',
...
const MOCK_SCHEDULES = [
  { id: 'sch-1', activityId: 'act-3', outlineId: 'out-1', currentWeek: 8, time: '10:00am - 12:30pm', staff: ['user-1'], clients: ['Alice', 'Bob', 'Charlie'] },
  { id: 'sch-2', activityId: 'act-1', outlineId: 'out-2', currentWeek: 12, time: '1:30pm - 3:30pm', staff: ['user-1'], clients: ['David', 'Eve'] },
]
...
export const useStore = create<State>((set) => ({
  ...
  commBoards: [
    {
      ...
      cols: 3,
      cells: [
        { id: 0, text: 'Yes', image: '', bgColor: '#dcfce3', textColor: '#000000' },
        { id: 1, text: 'No', image: '', bgColor: '#fee2e2', textColor: '#000000' },
        { id: 2, text: 'Stop', image: '', bgColor: '#fee2e2', textColor: '#000000' },
        { id: 3, text: 'More', image: '', bgColor: '#ffffff', textColor: '#000000' },
        { id: 4, text: 'Drink', image: '', bgColor: '#dbeafe', textColor: '#000000' },
        { id: 5, text: 'Eat', image: '', bgColor: '#fef08a', textColor: '#000000' },
      ]
    }
  ],
  ...
  addCommBoard: (board) => set((state) => ({ commBoards: [...state.commBoards, board] })),
  updateCommBoard: (id, updates) => set((state) => ({
    commBoards: state.commBoards.map(b => b.id === id ? { ...b, ...updates } : b)
  })),
  deleteCommBoard: (id) => set((state) => ({
